...
class SelectContext(IntEnum):
    ...
    SETUP_ACTIVE_POKEMON = (
        1,
    )  # Card. Select the Pokémon to put into your Active Spot during Set Up.
    SETUP_BENCH_POKEMON = (
        2,
    )  # Card. Select the Pokémon to put onto your Bench during Set Up.
    ...
    TO_DECK_BOTTOM = (
        10,
    )  # Card. Select the card to return to the bottom of your deck.
    ...
    DAMAGE_COUNTER_ANY = (
        14,
    )  # Card. Select the Pokémon to place damage counters on using the effect that lets you place them as you like.
    DAMAGE = (15,)  # Card. Select the Pokémon to deal damage.
    REMOVE_DAMAGE_COUNTER = (
        16,
    )  # Card. Select the Pokémon to remove damage counters from.
    ...
    DISCARD_CARD_OR_ATTACHED_CARD = (
        29,
    )  # CardOrAttachedCard. Select the card to discard.
    ...
    EVOLVE = (
        37,
    )  # Evolve. Select the Pokémon that is the evolution source and the Pokémon that is the evolution target.
    ...
    REMOVE_DAMAGE_COUNTER_COUNT = (
        40,
    )  # Count. Select how many damage counters to remove.
    ...
    AFFECT_SPECIAL_CONDITION = (
        47,
    )  # SpecialCondition. Choose the special condition to affect.
    RECOVER_SPECIAL_CONDITION = (
        48,
    )  # SpecialCondition. Choose the special condition to recover.
...
class PlayerState:
    active: list[
        Pokemon | None
    ]  # Active Pokémon (None if the card is facedown). The array size is either 0 or 1.
    ...
    prize: list[
        Card | None
    ]  # Prize cards (None if the card is facedown). The first element is the bottom of the prize, and the last element is the top.
...
class State:
    ...
    looking: (
        list[Card | None] | None
    )  # Looking cards (None if the card is facedown). None if not looking cards.
    players: list[
        PlayerState
    ]  # An array of player states. The number of elements is 2.
...
class SelectData:
    ...
    deck: (
        list[Card] | None
    )  # An array of cards; None unless selecting cards from the deck.
    contextCard: (
        Card | None
    )  # Which card is the selection concerning? This is sent when the context is "Activate"; otherwise, it is null.
    effect: (
        Card | None
    )  # The card that is activating the effect currently being processed.
...
class Observation:
    select: (
        SelectData | None
    )  # Selection information. At the time of the initial deck selection, it will be None.
    logs: list[Log]  # Events that have occurred since the last selection.
    current: (
        State | None
    )  # Current state. At the time of the initial deck selection, it will be None.
...
class CardData:
    ...
    aceSpec: (
        bool  # True if ACE SPEC. You can't have more than 1 ACE SPEC card in your deck.
    )
    evolvesFrom: (
        str | None
    )  # If the Pokémon has evolved, then the name of its pre-evolution. Otherwise, None.
...
def search_step(search_id: int, select: list[int]) -> SearchState:
    ...
    """
    bs = lib.SearchStep(
        agent_ptr, search_id, (ctypes.c_int * len(select))(*select), len(select)
    )
    result = json_to_dataclass(bs, ApiResult)
    if result.error != 0:
        if result.error == 1:
