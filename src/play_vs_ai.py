import json
    ...
    try:
        from baseline import MyModel, mcts_agent, sample_deck, card_table, to_observation_class, battle_start, battle_finish, battle_select, visualize_data
    except ImportError:
    ...
    OPTION_TYPES = {
        ...
        5: "ENERGY_CARD", 6: "ENERGY", 7: "PLAY CARD", 8: "ATTACH CARD",
        9: "EVOLVE POKEMON", 10: "USE ABILITY", 11: "DISCARD CARD", 12: "RETREAT",
        13: "ATTACK", 14: "END TURN", 15: "SKILL", 16: "SPECIAL CONDITION"
    }

    AREA_TYPES = {
        1: "DECK", 2: "HAND", 3: "DISCARD", 4: "ACTIVE", 
        5: "BENCH", 6: "PRIZE", 7: "STADIUM", 8: "ENERGY",
        9: "TOOL", 10: "PRE_EVOLUTION", 11: "PLAYER", 12: "LOOKING"
    }
    ...
    def process_ai_turns():
        ...
        while GAME_SESSION["is_active"]:
            ...
            if obs["current"]["result"] >= 0:
                GAME_SESSION["logs"].append(f"Match over. Result code: {obs['current']['result']}")
    ...
    class UnifiedWorkspaceHandler(BaseHTTPRequestHandler):
        ...
        def send_json_state(self):
            ...
            for idx, opt in enumerate(obs.get("select", {}).get("option", [])):
                ...
                card_name = card_table[card_id].name if (card_id and card_id in card_table) else None
                
                details = {}
                for k, v in opt.items():
                    if k == "type": 
                        continue
                    if k in ["area", "inPlayArea"] and isinstance(v, int):
                        details[k] = AREA_TYPES.get(v, f"AREA_{v}")
                    elif k == "cardId" and isinstance(v, int) and v in card_table:
                        details[k] = f"{card_table[v].name} (ID {v})"
                        if not card_name: card_name = card_table[v].name
                    else:
                        details[k] = v
            ...
            if os.path.exists("vis.json"):
                try:
                    with open("vis.json", "r", encoding="utf-8") as f:
                        obj = json.load(f)
                    payload = json.dumps(obj["steps"][0][0]["visualize"]) if "steps" in obj else json.dumps(obj)
                except Exception:
                    pass
